...
router
    ...
    .get(async (request, response) => {
        ...
        try {
            ...
            if (result && result.saved_tips) {
                return response
                    .status(200)
                    .json(result.saved_tips);
            } else {
                return response
                    .status(404)
                    .json({"error": "user or saved_tips not found"});
            }
        } catch (error) {
            console.log(error);
        }
...
router
    ...
    .post(async (request, response) => {
        ...
        try {
            ...
            const result = await get_tips(username);
            if (result && result.saved_tips) {
                var previous_tips = result.saved_tips;
        ...
        try {
            ...
            if (added) {
                return response
                    .status(200)
                    .json({"success": "the tip was added"});
            } else {
                return response
                    .status(500)
...
router
    ...
    .post(async (request, response) => {
        ...
        console.log('POST request to path /tips/delete/:username/:id');
        ...
        try {
            const removed = await update_tips(username, new_tips);
            if (removed) {
                return response
                    .status(200)
